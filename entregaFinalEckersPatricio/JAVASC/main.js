...
finalizarBtn.addEventListener("click", () => {
  ...
  Swal.fire({
    ...
    if (metodoPago === "tarjeta") {
      htmlFormulario = `
        <input id="nro" class="swal2-input" type="number" placeholder="Número de tarjeta">
        <input id="titular" class="swal2-input" type="text" placeholder="Nombre del titular">
      `;
    } else if (metodoPago === "debito") {
      htmlFormulario = `
        <input id="nroDebito" class="swal2-input" type="number" placeholder="Número de tarjeta débito">
    ...
    Swal.fire({
      ...
      preConfirm: () => {
        // Capturamos todos los inputs que se generaron en el HTML de arriba
        ...
        inputs.forEach(input => {
          if (!input.value.trim()) {
            mjeError = "Todos los campos son obligatorios";
          }
        });

        if (mjeError) {
          Swal.showValidationMessage(mjeError);
          return false; // Evita que la alerta avance
        }
        return true; // Todo OK
      }
    }).then((datosOk) => {
